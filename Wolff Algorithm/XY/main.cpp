...
void work()
{
    while (true)
    {
        ...
        // generate samples
        vector<xy_t> batch = generate_samples(L, T, batch_size);
...
int main(int argc, char* argv[]) // usage: <name>.exe <N_samples> <L> <T> <threads> <batch_size> <output_path> 
{
    if (argc < 6)
    ...
    L = stoi(argv[2]);
    T = stof(argv[3]);
    unsigned int n_threads = stoi(argv[4]);
    batch_size = stoi(argv[5]);
    string target_file(argv[6]);

    if (n_threads < 1)
        n_threads = 1;
    cout << "generating " << N << " samples of L = " << L << ", T = " << T << endl;
