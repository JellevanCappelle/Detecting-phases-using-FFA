...
class FFLayer(Layer):
    def __init__(self, size: int, T: Optional[float], mean_delay: float, mean_weight: float, weight_cost: float, **kwargs):
        """`mean_delay` must be in the (0, 1] range"""

        super().__init__(**kwargs)
        self.size = size
        self.means = tf.Variable(0.5 * tf.ones((size,)), trainable = False)
        self.T = T if T is not None else tf.sqrt(tf.cast(size, tf.float32))
    ...
    def probability(self, state):
        ...
        x = (self.goodness(state) - threshold) / self.T
        return 1 / (1 + tf.exp(-x))
...
class FFModel(Model):
    ...
    def __init__(self, layer_sizes: list, min_output_layer: int = 1,
        ...
        self.fflayers = [FFLayer(size, T, mean_delay, mean_weight, weight_cost)
    ...
    @tf.function
    def call(self, x):
        """return the hidden states. `x` must be a column of input row-vectors."""
