...
class FFScalarDiscriminator(Model):
    def __init__(self, ffmodel: FFModel, bins_start: float, bins_end: float, n_bins: int, offset_scale: float):
        if n_bins < 2:
        ...
        self.offset_scale = offset_scale if offset_scale is not None else self.bin_width
        self.bins = tf.linspace(bins_start, bins_end, n_bins)
