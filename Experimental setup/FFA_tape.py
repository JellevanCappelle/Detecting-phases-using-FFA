...
class FFLayer(Layer):
    ...
    def calc_state_and_gradients(self, pos_inputs, neg_inputs):
        ...
        with tf.GradientTape() as tape:
            ...
            # calculate loss
            pos_log = tf.math.log(1 + tf.math.exp(self.size - pos_goods))
            neg_log = tf.math.log(1 + tf.math.exp(self.size - neg_goods))
            ffa_loss = tf.reduce_mean(pos_log + neg_log + neg_goods) / 2

            # update means, calculate mean-of-means, add peer-normalization gradients to the loss
            ...
            peer_loss = tf.reduce_sum(tf.stop_gradient(mean_of_means - self.means) * pos_state) # (this is a bit hacky)
            peer_loss *= self.mean_weight / tf.cast(tf.shape(pos_inputs)[0], tf.float32)

            loss = ffa_loss + peer_loss
...
class FFModel(Model):
    ...
    def train_step(self, data):
        ...
        for i, layer in enumerate(self.fflayers):
            ...
            pos_state, neg_state, grads, err = layer.calc_state_and_gradients(pos_norm, neg_norm)
